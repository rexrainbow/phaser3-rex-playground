...
var require$$0$1 = require('directory-tree');
...
var LoaderTypes_1 = LoaderTypes$2;
...
var PackAssetFile$1 = function (type, child, config, totalPackResults) {
...
var GetKey = function (name) {
    return path$1.parse(name).name;
};

var GetExtend = function (name) {
    return path$1.extname(name);
};
...
var PackAssetFile_1 = PackAssetFile$1;

const PackAssetFile = PackAssetFile_1;

var PackAssetFolder$1 = function (tree, config) {
    var type = tree.name;
...
var PackAssetFolder_1 = PackAssetFolder$1;

const PackAssetFolder = PackAssetFolder_1;
const LoaderTypes$1 = LoaderTypes_1;

var PackSubFolders$1 = function (tree, config) {
    var subFolders = tree.children.filter(function (child) {
        return child.type === 'directory';
    });

    var assetsFolders = subFolders.filter(function (child) {
        return (LoaderTypes$1.indexOf(child.name) !== -1);
    });
    ...
    if (assetsFolders.length > 0) {
        ...
        subFolders.forEach(function (child) {
            var name = child.name;
            if (LoaderTypes$1.indexOf(name) === -1) {
                result[name] = PackSubFolders$1(child, config);
            }
        });
    }
...
var PackSubFolders_1 = PackSubFolders$1;

const dirTree = require$$0$1;
const LoaderTypes = LoaderTypes_1;
const PackSubFolders = PackSubFolders_1;
...
var FolderToJSON$1 = function (root, config) {
    if (config === undefined) {
        config = {};
    }
...
var FolderToJSON_1 = FolderToJSON$1;

const FolderToJSON = FolderToJSON_1;
const fs = require$$1;
